...
class Pixel {
    ...
    render(elapsed: number): void {
        if (this.pixelGraphic.alpha <= 0) { this.destory(); return; }
        // Animate
        if (Math.floor(elapsed) % 2 === 1) { this.pixelGraphic.y -= 0.3; } // Y pos
...
export class EffectDarkness {
    ...
    containerSettings(): void {
        ...
        let boundsGraphic = new PIXI.Graphics();
            if (this.debug) { boundsGraphic.beginFill(0xff0000, 0.5); }
    ...
    generatePixels(): void {
        ...
        setInterval(() => { this.pixelArr.push(new Pixel({ container: this.container, pixelArr: this.pixelArr })); }, 100);
    }
